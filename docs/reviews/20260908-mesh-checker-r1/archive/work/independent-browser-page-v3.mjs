...
export async function run(){
 ...
 try{
  ...
  const svg='<svg xmlns="http://www.w3.org/2000/svg" width="20" height="10" viewBox="0 0 20 10"><path fill="#ff0000" d="M0 0H10V10H0Z"/><path fill="#0000ff" d="M10 0H20V10H10Z"/></svg>';
  source=await client.build({kind:'svg',source:svg,thicknessMm:2,longEdgeMm:20,toleranceMm:.001},{generation:++generation});
  ...
  const unionPending=client.finalSceneGeometry(source,{revision:'1',expectedRevision:'1',mapping},{generation:++generation});for(let i=0;i<10&&!client.active;i++)await Promise.resolve();need(client.active,'CANCEL_NOT_DISPATCHED');await client.cancel();await expectReject('native-union-cancel',unionPending,['CANCELLED']);
 ...
 }catch(error){return {crossOriginIsolated,trace,artifacts,failure:error.message,capabilities:client.serviceCapabilities};}finally{source?.release();await checker.reset();client.dispose();}
}
