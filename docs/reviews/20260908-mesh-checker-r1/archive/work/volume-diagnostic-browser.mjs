...
for(const file of ['independent-browser-page.mjs','analytic-shapes.mjs'])routes.set('/review/'+file,fs.readFileSync(path.join(work,file==='independent-browser-page.mjs'?'volume-diagnostic-page.mjs':file)));
...
try{for(const engine of (process.env.REVIEW_BROWSER_ENGINES??'chromium,firefox,webkit').split(',')){
 let context;try{
  context=await pw[engine].launchPersistentContext(path.join(run,'cache/bp',tag+'-'+engine[0]),{headless:true,acceptDownloads:false,downloadsPath:path.join(run,'temp',tag+'-'+engine),serviceWorkers:'block',...(engine==='firefox'?{firefoxUserPrefs:{'network.proxy.type':0}}:{})});
  await context.route('**/*',route=>route.request().url().startsWith(origin+'/')?route.continue():route.abort());const page=process.env.REVIEW_BROWSER_PAGE==='new'?await context.newPage():(context.pages()[0]??await context.newPage()),errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto(origin);
  ...
  result.browser=context.browser().version();result.harness={profile:path.join(run,'cache/bp',tag+'-'+engine[0]),pageMode:process.env.REVIEW_BROWSER_PAGE??'initial'};result.errors=errors;fs.writeFileSync(path.join(dest,engine+'.json'),JSON.stringify(result,null,2));if(result.failure)throw Error(result.failure);if(errors.length||!result.crossOriginIsolated)throw Error('BROWSER_SECURITY_OR_SCRIPT');console.log(JSON.stringify({engine,browser:result.browser,checks:result.trace.map(t=>({name:t.name,verdict:t.verdict,code:t.code,volumeMm3:t.volumeMm3}))}));
