...
const root=fs.realpathSync(process.env.PROJECT_ROOT),run=fs.realpathSync(process.env.PROJECT_REVIEW_RUN),base=path.join(run,'work/review-source');
...
function add(dir,prefix){for(const e of fs.readdirSync(dir,{withFileTypes:true})){const p=path.join(dir,e.name);if(e.isDirectory())add(p,prefix+'/'+e.name);else if(e.name.endsWith('.mjs')&&!p.includes(path.sep+'backend'+path.sep)&&!p.includes(path.sep+'tests'+path.sep)&&!p.includes(path.sep+'tools'+path.sep))routes.set(prefix+'/'+e.name,fs.readFileSync(p));}}
...
for(const name of engines)test('real root + validator Workers '+name,{timeout:600000},async()=>{
 ...
 const profile=path.join(run,'cache/browser-profiles',tag+'-'+name[0]),context=await pw[name].launchPersistentContext(profile,{headless:true,downloadsPath:path.join(run,'temp/downloads-'+tag+'-'+name),acceptDownloads:false,serviceWorkers:'block',...(name==='firefox'?{firefoxUserPrefs:{'network.proxy.type':0}}:{})});
