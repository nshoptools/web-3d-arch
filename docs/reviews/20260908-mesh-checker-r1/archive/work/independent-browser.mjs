...
const routes=new Map(),hash=b=>createHash('sha256').update(b).digest('hex');
...
function addModule(file,url){
 ...
 for(const match of text.matchAll(/(?:from\s*|import\s*\(\s*|new URL\(\s*)['"](\.{1,2}\/[^'"]+\.mjs)['"]/g)){
  const dep=path.resolve(path.dirname(real),match[1]),depURL=new URL(match[1],'http://localhost'+url).pathname;addModule(dep,depURL);
 }
}
...
for(const file of ['independent-browser-page.mjs','analytic-shapes.mjs'])routes.set('/review/'+file,fs.readFileSync(path.join(work,file)));
...
fs.writeFileSync(path.join(dest,'route-manifest.json'),JSON.stringify({headers,files:[...routes].map(([url,b])=>({url,bytes:b.length,sha256:hash(b)}))},null,2));
...
try{for(const engine of (process.env.REVIEW_BROWSER_ENGINES??'chromium,firefox,webkit').split(',')){
 let context;try{
  ...
  result.browser=context.browser().version();result.errors=errors;fs.writeFileSync(path.join(dest,engine+'.json'),JSON.stringify(result,null,2));if(errors.length||!result.crossOriginIsolated)throw Error('BROWSER_SECURITY_OR_SCRIPT');console.log(JSON.stringify({engine,browser:result.browser,checks:result.trace.map(t=>({name:t.name,verdict:t.verdict,code:t.code,volumeMm3:t.volumeMm3}))}));
