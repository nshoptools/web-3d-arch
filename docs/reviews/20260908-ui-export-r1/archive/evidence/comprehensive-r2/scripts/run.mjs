import fs from 'node:fs';import path from 'node:path';import {pathToFileURL} from 'node:url';import {env,write,safe} from './support.mjs';import {cases} from './cases.mjs';import {extraCases} from './extra-cases.mjs';
...
const record=async result=>{checks.push(result);write(path.join(out,name+'-checks.json'),checks);console.log(name+' '+(result.pass?'PASS ':'FAIL ')+result.id+': '+result.name)};await cases(page,record,out,name);await extraCases(page,record,out,name);
