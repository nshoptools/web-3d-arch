...
import {readSnapshot,partMesh,inspectMesh,bbox,intersections,close} from './oracles/spatial-oracle.mjs';
import {evidence} from '../tools/test-env.mjs';
const read=p=>JSON.parse(fs.readFileSync(path.join(evidence,p))),a=read('source-native-results.json'),b=read('source-wasm-results.json');
...
for(const row of a.cases){const id=row.id,n=read(`tests-native/${id}.json`),w=read(`tests-wasm/${id}.json`);
  ...
  const ns=readSnapshot(fs.readFileSync(path.join(evidence,`tests-native/${id}.bin`))),ws=readSnapshot(fs.readFileSync(path.join(evidence,`tests-wasm/${id}.bin`)));assert.equal(ns.parts.length,ws.parts.length);
...
fs.writeFileSync(path.join(evidence,'native-wasm-parity.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify({...result,records:undefined}));
