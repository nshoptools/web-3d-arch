...
struct Builder {
  ...
  CrossSection regularize_support(const CrossSection&s){
    // Library simplification of derived support only. Verify each retained
    // chord against its entire original chain, not just the library epsilon.
    // Artwork/shared material source edges never enter this operation.
    Polygons polys;double bound=0;tick();
    for(const auto&poly:s.ToPolygons()){
      ...
      auto result=Clipper2Lib::SimplifyPath(original,2.0,true);need(result.size()>=3,"SUPPORT_REGULARIZATION_COLLAPSE");
      std::map<P,size_t>positions;for(size_t i=0;i<original.size();i++)need(positions.emplace(P{original[i].x,original[i].y},i).second,"SUPPORT_REGULARIZATION_POINT_CONTACT");
      SimplePolygon normalized;
      for(size_t i=0;i<result.size();i++){
        auto a=result[i],b=result[(i+1)%result.size()];auto begin=positions.at({a.x,a.y}),end=positions.at({b.x,b.y});double dx=double(b.x-a.x),dy=double(b.y-a.y),den=dx*dx+dy*dy;need(den>0,"SUPPORT_ZERO_CHORD");
        for(size_t j=begin;j!=end;j=(j+1)%original.size()){auto q=original[j];double t=std::clamp(((q.x-a.x)*dx+(q.y-a.y)*dy)/den,0.,1.);bound=std::max(bound,std::hypot(q.x-a.x-t*dx,q.y-a.y-t*dy)/G);}
        normalized.push_back({a.x/G,a.y/G});
      }polys.push_back(normalized);
    }
    need(bound<=2.000001/G,"SUPPORT_REGULARIZATION_BOUND",0,0,AS_RESOURCE);o.errors.push_back({7,0,0,-bound-std::sqrt(2.)/2/G,bound+std::sqrt(2.)/2/G});
