#ifndef ARCH_RASTER_RUNTIME_V1_H
#define ARCH_RASTER_RUNTIME_V1_H
#include <stdint.h>
/*
 * Implemented extension of root runtime ABI 2, raster ABI 1.
 * Read docs/runtime-binding.md for byte layouts, limits and ownership.
 * One Worker calls these functions serially; the root atomic control is shared.
 * No unregistered pointer/offset, filesystem I/O, callbacks or second allocator.
 * All wire blocks are LE, exact-size and field-validated (never cast to struct).
 */
#ifdef __cplusplus
extern "C" {
#endif
 ...
 * Zero handle selects fixed runtime defaults; aggregate admission may still fail.
 */
 ...
 * the next prepare/confirm/build; it is Worker-local metadata, never a retained lease.
 */
enum ArchRasterErrorCode {
    ...
    ARCH_RASTER_INTEGRITY=104, ARCH_RASTER_HANDLE_INVALID=105,
    ARCH_RASTER_GEOMETRY_FAILED=106, ARCH_RASTER_QUANTIZATION_UNCERTIFIED=107,
    ARCH_RASTER_EMPTY_CONTEXT=108
};
uint32_t arch_raster_error_code(void);
/* Aggregate root logical charged bytes + transient reservations; not RSS. */
uint32_t arch_raster_owned_bytes(void);
uint32_t arch_raster_abi_version(void); /* 1 */
 ...
 * verifies the sealed immutable source/geometry and returns an accepted lease.
 * Repeating confirm reuses the same accepted ID while leased, adding ONE lease.
 * All prepare results require confirm before build, even status=Ready.
 * Confirmation is tied to the runtime SHA in summary[128..160], not the source
 * crate hash (which is preserved separately in metadata TLV tag 23).
 ...
 * changing size requires prepare+confirm for a new proposal hash.
 * Root uses the frozen global kind28 table and existing controlled C++ path.
 * Parent validates domain state/SHA assets and performs subsequent assembly.
 * This builds flat source material extrusions, not a full product. Returns the
 ...
 * Aligned, immutable buffers remain owned for the lease lifetime.
 */
enum ArchRasterBufferKind {
    ARCH_RASTER_SUMMARY=1,       /* header described in runtime-binding.md */
    ...
    ARCH_RASTER_MERGES=19,          /* 32-byte decision records */
    ARCH_RASTER_OPTIONS=20,         /* exact OptionsV2, 200 LE bytes */
    ARCH_RASTER_EFFECTIVE_LIMITS=21,/* exact effective LimitsV1, 104 bytes */
    ARCH_RASTER_RENDER_ORIGIN=22,   /* original registered origin block */
    ARCH_RASTER_REGIONS_U32=23,     /* stride48: see binding */
    ARCH_RASTER_SOURCE_LOOPS_U32=24,/* stride16: first,count,region,label */
    ARCH_RASTER_SOURCE_DIRECTED_U32=25,/* source edgeID*2+reversed */
    ARCH_RASTER_ADJACENCY_U32=26,   /* pairs of labels */
    ARCH_RASTER_PINNED_SOURCE_U32=27,/* source vertex IDs */
    ARCH_RASTER_MM_XY_I64=28,       /* global 1nm table, same derived IDs */
    ARCH_RASTER_LOOP_CHAINS_U32=29, /* chainID*2+reversed */
    ARCH_RASTER_LOOP_CHAIN_RANGES=30,/* sourceRegion,sourceLoop,first,count */
    ARCH_RASTER_EXIF_U8=31          /* exact original EXIF if present */
};
