...
const cap=(id,available,reason)=>({id,available:!!available,...(!available?{reason:typeof reason==='string'&&reason.trim()?reason:'ADAPTER_CAPABILITY_REQUIRED'}:{})});
...
export class AppController {
 constructor({origin,deviceId,adapters={},fetchImpl,storeFactory=openProjectStore,clock=Date.now,deviceMode='private',navigate=url=>globalThis.location.assign(url),objectURLs=globalThis.URL,storagePolicy={},leasePolicy='signed-offline',allowAuthenticatedLeaseResponse=false}){
  ...
  this.storageEstimate={usedBytes:0,quotaBytes:null,estimate:true,warning:'Storage estimate unavailable'};
 ...
 report(e){if(e.code==='HISTORY_PRUNING_REQUIRED'&&this.pendingChange)e.confirmation={title:'Reduce retained history',changes:['Preserve current snapshot; prune oldest retained transactions to the budget'],retry:{type:'proposal.accept',id:this.pendingChange.id,confirmed:true}};const d=diagnostic(e);this.diagnostics=this.diagnostics.slice(-19).concat(d);this.emit();return {ok:false,diagnostic:d,...(e.confirmation?{confirmation:e.confirmation}:{})};}
 ...
 makeSnapshot(){
  ...
  const canvasReason=!canEdit?'Authorize the current user and unlock this project before editing.':!r?'Convert this source to raster with explicit confirmation before editing.':'An available editing Worker adapter is required.';
  ...
  return freeze({contractVersion:'0.3',environment:'application',version:String(this.version),online:this.online,capabilities:caps,session:{...this.session,...(this.store&&!this.store.status().canEdit&&this.session.user?{status:'expired'}:{})},
   ...
   job:this.job?{id:this.job.id,state:this.job.state,stage:this.job.stage,progress:this.job.progress,cancellable:true}:null,diagnostics:[...this.diagnostics],library:[...this.library],exports,exportReceipts:[...this.exportReceipts.values()].map(r=>r.receipt.view).reverse(),aiProviders:providers,printer:printer??{id:null,label:'No printer selected',filamentSlots:0,qualified:false},
