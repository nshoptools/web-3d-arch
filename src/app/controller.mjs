...
export class AppController {
 ...
 invalidate(status='signed-out',{rescue=false}={}){
  ...
  this.doc=null;this.assets=new Map();this.projectId='';this.headRevision=0;this.workspaceStep=1;this.library=[];this.libraryCache.clear();this.unrecognizedProjects=[];this.printers=[];this.pendingChange=null;this.rawImport=null;this.selection=null;this.editorTool=null;this.conflicts=[];this.readOnly=false;this.mirror=null;
 ...
 makeSnapshot(){
  ...
  return freeze({contractVersion:'0.3',environment:'application',version:String(this.version),online:this.online,capabilities:caps,session:{...this.session,...(this.store&&!this.store.status().canEdit&&this.session.user?{status:'expired'}:{})},
   ...
   printerProfiles:this.profileLibrary?.snapshot(),settings:this.remote?.settingsViews()??[],editor:{...(a?.editor??data(DEFAULT_EDITOR)),...(a&&this.editorTool?{tool:this.editorTool}:{})},aiJobs:[...(this.remote?.jobs.values()??[])].map(j=>this.remote.jobView(j)),aiBudget:{revision:this.remote?.budget.revision??0,limits:(this.remote?.budget.money??[]).map(b=>({currency:b.currency,perOperation:moneyText(b.perOperationMicros),perDay:moneyText(b.perDayMicros),perMonth:moneyText(b.perMonthMicros)}))},
