...
fn main(){
 ...
 for case in data["cases"].as_array().unwrap(){
  ...
  assert_eq!(unsafe{archmi_stage_stl(id,xyz.as_ptr(),(xyz.len()/9)as u32,1,0x30353bff,1,label.as_ptr(),mat.as_ptr(),matname.as_ptr(),case["conversionError"].as_f64().unwrap())},0);
  ...
  cmd["version"]=json!("arch-mesh-csg-request/1");cmd["snapshotGeneration"]=json!(1);cmd["bindings"]=json!(binds);cmd["materials"]=json!(mats);cmd["targets"]=json!([binds[target]["semanticId"]]);
