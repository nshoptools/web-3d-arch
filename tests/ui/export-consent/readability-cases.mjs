import path from 'node:path';import assert from 'node:assert/strict';import {write,openFolds} from './support.mjs';
export async function readabilityCases(page,record,out,engine){
for(const [width,fontSize]of [[1280,14],[1280,28],[320,28]]){
const id='V02-'+width+'-'+fontSize;await page.setViewportSize({width,height:900});await page.evaluate(()=>window.review.reset());await openFolds(page);await page.evaluate(size=>window.review.update({settings:[{key:'fontSize',label:'Text size',value:String(size)}]}),fontSize);await page.evaluate(()=>window.review.section('export'));
