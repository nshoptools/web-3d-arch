...
async retireAndClick(kind:string,label:string){await change(()=>{if(kind==='account'){mock.change('B');mock.change('A')}else{mock.change(undefined,'P2');mock.change(undefined,'P1')}const button=Array.from(document.querySelectorAll('button')).find(e=>e.textContent?.trim()===label);if(!button)throw Error('Missing action '+label);button.click()})},
