import fs from 'node:fs';import path from 'node:path';import {pathToFileURL} from 'node:url';import {env,write,safe,verify} from './support.mjs';import {cases} from './cases.mjs';import {extraCases} from './extra-cases.mjs';import {followupCases} from './followup-cases.mjs';import {readabilityCases} from './readability-cases.mjs';import {keyboardCases} from './keyboard-cases.mjs';
...
for(const name of (process.env.REVIEW_ENGINES??'chromium,firefox,webkit').split(',')){let context;const checks=[],errors=[],consoleMessages=[],denied=[];try{
const profile=safe(run,path.join(run,'p',label+'-'+name[0]));const downloads=safe(run,path.join(run,'work/browser-downloads',label,name));fs.mkdirSync(downloads,{recursive:true});
...
const record=async result=>{checks.push(result);write(path.join(out,name+'-checks.json'),checks);console.log(name+' '+(result.pass?'PASS ':'FAIL ')+result.id+': '+result.name)};if(process.env.REVIEW_ONLY==='lifecycle')await cases(page,record,out,name);else if(process.env.REVIEW_ONLY==='keyboard')await keyboardCases(page,record,out,name);else if(process.env.REVIEW_ONLY==='readability')await readabilityCases(page,record,out,name);else if(process.env.REVIEW_ONLY==='followup')await followupCases(page,record,out,name);else{await cases(page,record,out,name);await extraCases(page,record,out,name);}
...
write(path.join(out,'results.json'),{at:new Date().toISOString(),sourceManifest:c.manifestHash,implementationOnly:true,scope:'Actual React with controlled bridge; independent reviewer cases reused by parent, no native/backend qualification',packages:Object.fromEntries(['react','react-dom','vite','typescript','playwright'].map(n=>[n,JSON.parse(fs.readFileSync(path.join(deps,n,'package.json'),'utf8')).version])),results});
verify();process.exitCode=results.some(r=>r.fatal||r.errors.length||r.denied.length||r.consoleMessages.length||r.checks.some(t=>!t.pass))?1:0;
