...
export const client={epoch:1,disposed:false,worker:{testOnly:true},memory:M.HEAPU8.buffer,serviceCapabilities:{raster:true,sourceFrameVersion:M._arch_source_frame_version(),geometryVersions:{mechanicsAbi:2,mechanicsSemantics:3,sourceAbi:1,sourceSemantics:2,datumExtension:1}},onRetirement:()=>()=>{},
 ...
 async sourceFrame(original,request,{generation:g}){reset(g,'sourceFrame');
  const apply=(id,sg)=>applySourceFrame(M,id,sg,request,g);
  const id=original?.kind==='raster-token'?dispatcher.withSourceReference(original.token,ref=>{if(ref.kind!=='snapshot')throw Error('SOURCE_FRAME_KIND');return apply(ref.id,ref.generation);}):(roots.has(original)?apply(original.id,original.generation):(()=>{throw Error('SNAPSHOT_LEASE_OWNERSHIP');})());
  let ended=false;const metadata=JSON.parse(new TextDecoder().decode(M.HEAPU8.slice(M._arch_metadata_ptr(id),M._arch_metadata_ptr(id)+M._arch_metadata_len(id))));
  const l={id,metadata,epoch:client.epoch,generation:g,bytes(){if(ended)throw Error('RELEASED');return M.HEAPU8.subarray(M._arch_snapshot_ptr(id),M._arch_snapshot_ptr(id)+M._arch_snapshot_len(id));},release(){if(!ended){ended=true;roots.delete(l);if(M._arch_snapshot_release(id)!==1)throw Error('RELEASE');}}};roots.add(l);return l;},
