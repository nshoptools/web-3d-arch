...
async function fixture(options,fn){const h=await setup(T,{profiles,...options});try{return await fn(h);}finally{h.close();assert.deepEqual(T.stats(),{liveTestRoots:0,releasedPointersRetired:true});}}
...
test('EXP-01 union of production canonical paint slabs covers 1500 mm³ (2D overlap resolved upstream); exact source lease survives',()=>fixture({index:0},async h=>{
...
test('EXP-01 final-section sequence preserves a through-opening at two explicit heights; in/back explicit',()=>fixture({index:3},async h=>{
 h.exportOptions['svg-section'].section={mode:'sequence',startMm:1,endMm:3,stepMm:2,units:'in',side:'back',color:'material'};
 const a=await h.exporter.export(h.input('svg-section')),o=sectionOracle(a.bytes);assert.equal(o.samples.length,2);near(o.samples[0].area,184/(25.4**2));near(o.samples[1].area,184/(25.4**2));
 assert.deepEqual(o.samples.map(s=>s.rings),[2,2]);assert.ok(o.width.endsWith('in'));await save('through-opening-sequence.svg',a);
}));
