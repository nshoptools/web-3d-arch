...
import {productionSource} from './export-production-fixtures.mjs';
...
export function serviceTransport(M){
 let generation=0;const roots=new Map();const printing=typeof M._arch3mf_abi_version==='function'?createUnifiedPrinting(M):null;
 ...
 const adoptRoot=(id,g,metadata)=>{let released=false;roots.set(id,false);return {id,generation:g,epoch:73,...(metadata?{metadata}:{}),
  bytes(){if(released)throw Object.assign(Error('SNAPSHOT_RELEASED'),{code:'SNAPSHOT_RELEASED'});return read(id);},
  release(){if(!released){released=true;roots.set(id,true);counters.rootReleases++;if(M._arch_snapshot_release(id)!==1)throw Error('test root release');}}};};
 const client={disposed:false,epoch:73,serviceCapabilities:{finalExport:M._arch_final_export_version?.()===1},
 ...
 return {M,client,counters,before,printing,next,adoptRoot,
  ...
  makeRoot(index){const g=next(),id=productionSource(M,index,g);if(!id)throw Error('test fixture '+index);return adoptRoot(id,g);},
  stats:()=>({liveTestRoots:[...roots.values()].filter(released=>!released).length,releasedPointersRetired:[...roots].filter(([,released])=>released).every(([id])=>M._arch_snapshot_ptr(id)===0)}),
 };
}
export async function setup(transport,{index=2,nativeRoot=null,profiles={},model=true,firstLayerHeight=.16}={}){
 const T=transport,root=nativeRoot??T.makeRoot(index),view=readArchSnapshot(root.bytes()),sourceBytes=new TextEncoder().encode(sourceSVG),rawHash=await sha256(sourceBytes);
