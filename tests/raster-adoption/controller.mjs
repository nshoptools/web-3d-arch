...
export async function runRasterController({scenario,kernel,catalog,assetURLs,origin,createEditingClient,capture=async()=>{},families=['svg','raster','text','emoji'],products=['keychain'],styles=['noi'],group='matrix',onContext=()=>{},recordCase=async()=>{},beforeDispose=async()=>{},signal}){
 ...
 try{
  await scenario({c,context,sources,contexts,product,transactions,ctl,newProject,prepare,confirmChain,snapshot,measured,trace,rows});
  return {rows,trace,caseTimes,storage:'labelled-memory-recorder-real-controller-history-CAS'};
