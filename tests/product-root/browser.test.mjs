...
add(path.join(base,'src'),'/src');add(path.join(base,'src/assets/harfbuzz/dist'),'/src/assets/harfbuzz/dist');add(path.join(base,'tests/product-root'),'/tests/product-root');
...
let activeEngine,activeGroup;const groups=envSelection(process.env),caseTimes=new Map();
...
const engines=process.env.ARCH_ROOT_ENGINES?.split(',')??['chromium','firefox','webkit'];assert.ok(engines.length&&engines.every(e=>['chromium','firefox','webkit'].includes(e)));
for(const engine of engines)for(const group of groups)test('fresh controller/probe/ASFR '+engine+' '+group.id,{timeout:LIMITS.groupMs},async t=>{
 ...
 try{
  ...
  const result=await page.evaluate(async args=>(await import('/tests/product-root/browser-page.mjs')).run(args),{catalog:fixture.catalog,assetURLs:fixture.assetRecords.map(r=>({...r,url:origin+'/library/'+r.sha256})),runtime:{engine,version:browser.browser().version()},integrity,moduleURL:pins.module.url,group});
