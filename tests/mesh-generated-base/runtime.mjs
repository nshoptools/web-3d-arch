...
const run=process.env.PROJECT_REVIEW_RUN;export const fixture=JSON.parse(fs.readFileSync(path.join(run,'inputs/source-fixture-v2.json'))),baseOut=path.join(run,'evidence/controller-node-'+(process.env.ARCH_ROOT_TAG??'exploratory'));let out=baseOut;fs.mkdirSync(out,{recursive:true});
...
export let live;const service=createEngineTextService(M,{catalog:fixture.catalog,assetURLs,origin,runtime:{engine:'node',version:process.versions.node}},{origin});
...
export function createEditingClient(){let editor;return {async initialize(input){editor=await createEditor(input);return editor.token();},prepare:(c,control)=>editor.prepare(c,control),commit:(id,token)=>editor.commit(id,token),dispose(){editor=null;}};}
test.after(async()=>{service.dispose();noOwned();await new Promise(r=>{server.closeAllConnections();server.close(r);});});

export function context(){return live;}
export function updateContext(value){live=value;setLive(value);}
export {assetURLs,origin,runControllerCases,versions};
