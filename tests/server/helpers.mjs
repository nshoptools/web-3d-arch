...
import { createBackend,SID } from '../../src/server/app.mjs';
...
export class TestProvider {
  constructor() {
    this.metadata={id:'test-provider',version:'test-v1',endpointId:'test-endpoint',endpointUrl:'https://example.com/test-images',testOnly:true,
      models:[{id:'test-image',version:'test-model-v1',referenceImages:false}],keyCheckCostMicros:0,prices:{version:'test-price-v1',date:'2026-09-08',testOnly:true}};
...
export class Client {
  ...
  async request(method,path,data,headers={}) {
    ...
    const response=await fetch(this.f.app.origin+path,{method,headers:h,...(data!==undefined?{body:typeof data==='string'?data:JSON.stringify(data)}:{}),redirect:'manual'});
...
export async function setup(t,options={}) {
  ...
  let now=Date.UTC(2026,8,8,0,0,0);const clock=()=>now;
